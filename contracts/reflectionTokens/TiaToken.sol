...
contract TIA is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    constructor(address _lcxAddress, address _uniswapV2Address) {
        require(
            _lcxAddress != address(0),
            "Address should not be zero address"
        );
        require(
            _uniswapV2Address != address(0),
            "Address should not be zero address"
        );
        LCX = _lcxAddress;
    ...
    function rebalanceAndBurn() external nonReentrant returns (bool) {
        ...
        uint256 rebalanceAmount = balanceInContract.mul(_rebalancingFee).div(
            _rebalancingFee.add(_burningFee)
        );
        uint256 burnAmount = balanceInContract.mul(_burningFee).div(
            _rebalancingFee.add(_burningFee)
        );
